...
def create_dataset(filename,inpath,outpath,dataname):
    #Train 加载数据的方式,无load_dict,有Rt
    dir_dataset = inpath
    print(filename)
    """Load a dataset."""
...
def create_pred_dataset(filename,path,dataname):
    #Transfrerlearning 预测集的加载数据方法,无load_dict,无RT; 有输出mol_dict
    #未采用！
    dir_dataset = path
    print(filename)
    """Load a dataset."""
...
def create_dataset_kfold(x,y,inpath,outpath,data_name,idata_name):
    #TransferLearning 训练集的加载数据方法,有load_dict,有RT
    dir_input =inpath + f'{data_name}' + "-"
    ...
    dir_dataset=outpath + idata_name
...
def transferlearning_dataset_predict(x,path,data_name):
    #Transferlearning 加载预测集方法，有load_dict,无RT; 无输出mol_dict
    dir_input = path+f'{data_name}' + '-'
    with open(dir_input + 'atom_dict.pickle', 'rb') as f:
        c=pickle.load(f)
        for k in c.keys():
            atom_dict.get(k)
