...
#' @export
Lipid_Unsaturation_plot <- function(data,
                                    savepath = "./",
                                    mapname = "Lipid_Unsaturation",
                                    wbsave = T,
  ...
  if("diff_filter" %in% class(data)){
    mat <- data
    plotdata <- merge(mat$args$info[,c("Sub Class","Total Unsaturation"),drop=F],
                      mat$args$data,by = 0,all = F)[,-1]
    data_C <- aggregate(.~`Sub Class`+`Total Unsaturation`,plotdata,sum) 
  ...
  resultp <- result[,c("Sub Class","Total Unsaturation","log2FoldChange")]
  resultlevel <- result[,c("Sub Class","Total Unsaturation","level")]
  plot <- dcast(resultp,`Sub Class`~`Total Unsaturation`,value.var = "log2FoldChange")
  plotlevel <- dcast(resultlevel,`Sub Class`~`Total Unsaturation`,value.var = "level")
...
if(this.path::is.main() & !is.null(whereami::thisfile_r())){
  ...
  map_lipid_unsaturationheatmap <- map_autodraw$new(moudle = Lipid_Unsaturation_plot)$draw
  ...
  # parser$add_argument("-dpi","--ppi",default = 300, help = "分辨率")
  ...
  # parser$add_argument("-sn","--savename",default = "绘图数据-Unsaturation.xlsx", help = "绘图数据保存名称")
  ...
  result <- do.call(what = map_lipid_unsaturationheatmap,args = args) 
}  
