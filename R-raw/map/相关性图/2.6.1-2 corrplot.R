...
corrplot <- function(corr,
                     ...
                     title = "\n Correlation",
                     tl.col = "black",
                     diag = ifelse(type == "full", T, F),
                     sig.level = c("", "", ""),
                     insig = "label_sig",
                     col = SelectColors(palette = "pinkblue",n = 100),
                     tl.cex = 10 / (4 + (dim(corr)[1]) / 8 + (max(nchar(row.names(corr)))) / 8),
                     type = ifelse(all(corr[row(corr) == col(corr)] == 1), "upper", "full"),
...
#' @export
map_common_corrplot <- function(mapmoudle = corrplot,
                                ...){
...
if(this.path::is.main() & !is.null(whereami::thisfile_r())){
  ...
  # 此图参数
  parser$add_argument("-c","--col",default = "pinkblue", help = "配色设置")
  parser$add_argument("-m","--mode",default = "mode1", help = "显著性呈现方式选择,可选mode1,mode2,mode3,mode3,其他",
                      choices = c("mode1","mode2","mode3","mode4"))
  parser$add_argument("-t","--type",default = "upper", help = "相关性图呈现方向，可选upper,full,lower",
                      choices = c("upper","full","lower"))
  ...
  parser$add_argument("-ac","--addCoef.col",default = "white", help = "标注显著性字体的颜色")
  ...
  corrdata <- do.call(what = map_common_corrplot,args = args)
