...
#' @export
splotmap2_obj <- function(data,infofile = NULL,...){
  if("mulstatistics" %in% class(data)){
    data <- getsplot_file(rdspath = data,infofile = infofile)
  }
  result <- splotmap2(data = data,...)
}
...
if(this.path::is.main() & !is.null(whereami::thisfile_r())){
  ...
  map_mulstatistics_splotmap2 <- map_autodraw$new(splotmap2_obj)$draw
  
  parser <- ArgumentParser()
  parser$add_argument("-f","--filename",nargs="+",
                      help = "splot矩阵文件",required = T)
  ...
  result <- do.call(what = map_mulstatistics_splotmap2,args = args) 
