...
#'
#' @export
datafetch_untargetgcms_obj <- function(data,
                                       ...){
  
  data$data$rawdata$data <- datafetch_untargetgcms(filename = data$info$datafrom$GC原始文件,
                                                   ...)
...
#'
#' @export 
datafetch_untargetgcms <- function(filename = NULL,
                                   samplename = NULL,
                                   grouping = NULL,
  ...
  if (mode == "fame") {
    ...
    n <- sum(dataFameCM == 0)
    if (n != 0) {
      stop(paste0("Fame内标异常:", n, "个内标为空值,请检查内标!!!检查无误后请使用fameaverage = T,后续将以该内标所有样本均值替代空值"))
    } else {
      print("内标无异常")
    }
  } else if (mode == "single") {
  ...
  if (mode == "area") {
    ...
    # 提取单内标
    savexlsx1(filename = savefile,sheet = singlefame,data = data3)
    # 归一化后ID和M文件合并并保存
  ...
  # 数据注释
  ciddata <- getmysqldata(dbname = "cosa",
                          table = "compound_structure",
                          tablelist =  "cid,inchikey",
                          wherename = "inchikey",
                          wheredata = data2$INCHIKEY)
  classification <- getmysqldata(dbname = "cosa",
                                 table = "compound_classification",
                                 tablelist =  "cid,superclass as `Super Class`,class as Class,subclass as `Sub Class`",
                                 wherename = "cid",
                                 wheredata = ciddata$cid)
  iddata <- getmysqldata(dbname = "cosa",
                         table = "compound_identifier",
                         tablelist =  "cid,hmdbid as HMDB,metlinid as METLIN,lipidmapsid as Lipidmaps,pubchemid as PubChem,keggid as kegg,casNumber as CAS",
                         wherename = "cid",
                         wheredata = ciddata$cid)
  
  iddata <- merge(iddata,classification,by = "cid",all = T)
  iddata <- merge(ciddata,iddata,by = "cid",all = T)
  iddata[is.na(iddata$`Super Class`),"Super Class"] <- "Unclassified"
  iddata[is.na(iddata$`Class`),"Class"] <- "Unclassified"
  iddata[is.na(iddata$`Sub Class`),"Sub Class"] <- "Unclassified"
  iddata <- iddata[,colnames(iddata) != "cid"]
  data3 <- merge(iddata,data2,by.x = "inchikey",by.y = "INCHIKEY",all = T)
