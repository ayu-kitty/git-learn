...
# 有问题
SERRF2 <- function(input = "C:\\Users\\Sili Fan\\Downloads\\Lin Lili\\TABLE 2-2017-10-16-PLASMA-UPPER-POS-FORMAT-ERROR.xlsx", ip = '2602:306:3144:1140:5994:7a10:7bfa:67ac'){
  ...
  readData = function(path =  "G:\\data\\D\\data project D.xlsx"){
    ...
    #### fData
    fData <- d[!is.na(d[,1]),c(which(is.na(d[1,])),sum(is.na(d[1,]))+1),drop = F] # The first row and column is critical of formating the data.
    ...
    fData. = do.call(cbind, lapply(fData., data.frame, stringsAsFactors=FALSE,check.names = F))
    ...
    pData. = do.call(cbind, lapply(pData., data.frame, stringsAsFactors=FALSE,check.names = F))
    ...
    eData <- data.frame(eData,stringsAsFactors = F,check.names = F)
