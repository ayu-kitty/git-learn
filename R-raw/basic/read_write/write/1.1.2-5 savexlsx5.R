...
#' 在workbook中添加sheet，减少频繁保存表格问题，首行加粗，列名保存，字体Arial
#'
#' @param data dataframe数据
...
addsheet5 <- function(data, 
  ...
  openxlsx::addStyle(wb, sheet = sheet, style = mode2, rows = 2:(dim(data)[1] + 1), cols = 1:(dim(data)[2] + 1), gridExpand = T)
