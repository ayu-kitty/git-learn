...
#'
#' @export
imzmlpreQualitative_2023 <- function(sample = NULL,
  ...
  for (mode in moderange) {
    ...
    if(length(samplename2) > 0){
      ...
      GetimzmlpreQualitative_2023(filename = filename,
                                  mode = mode,
                                  areards = areards,
                                  ...)
...
#' 
#' @export
GetimzmlpreQualitative_2023 <- function(filename = NULL,
                                        areards = NULL,
                                        mode = "neg",
                                        # 读取参数
                                        mass.range = NULL,
  ...
  if(!is.null(outdatabse)){
    if(outdatabse == "tcm" | outdatabse == "tcme"){
      hmdbdata <- getmysqldata(dbname = "cosa",table = "spatial_tcm")
    }else{
      stop("未找到相应数据库")
    }
  }else{
    hmdbdata <- getmysqldata(dbname = "cosa",table = "spatial_hmdb")
  }
  
  colnames(hmdbdata) <- c("cid","Formula","Adduct","mz","mode","Metabolites")
  hmdbdata$mz <- as.numeric(hmdbdata$mz)
  ...
  for ( i in 1:length(filename)) {
    ...
    if(is.null(mse)){mse <- mse_sample
    }else{mse <- BiocGenerics::cbind(mse, mse_sample)}
  ...
  sampledata <- readdata(filename = paste0(savepeakpath,"/样本离子统计.xlsx"),sheet = mode)
  sampledata$mz <- round(sampledata$mz,digits = 8)
  # mzmetadatatest <<- mzmetadata2
  mzmetadata2$mz <- round(sampledata$mz,digits = 8)
  mzmetadata2 <- merge(mzmetadata2,sampledata[,1:3],by="mz",all = T)
  mzmetadata3 <- mzmetadata2[order(mzmetadata2$samplemeandata,decreasing = T),]
  mzmetadata3 <- mzmetadata3[mzmetadata3$sampleselect,]
  mzmetadata3 <- mzmetadata3[!is.na(mzmetadata3$formula),]
  mzmetadata3 <- mzmetadata3[is.na(mzmetadata3$isotope),]
  mzmetadata3 <- mzmetadata3[is.na(mzmetadata3$polymer),]
  mzmetadata3 <- mzmetadata3[!duplicated(mzmetadata3$formula),]
  mzmetadata2[!(mzmetadata2$mz %in% mzmetadata3$mz), "sampleselect"] <- F
  ...
  # mzmetadata2 <<- mzmetadata2
  
  hmdbdata3 <- hmdbdata
  hmdbdata3 <- hmdbdata3[hmdbdata3$mode == mode,]
  ...
  qualitativedata <- merge(hmdbdata3,mzmetadata3[,c("formula","adducts")],by.x = c("Formula","Adduct"),by.y = c("formula","adducts"),all = F)
  qualitativedata <- qualitativedata[,c("cid","Formula","Adduct","mz","mode","Metabolites")]
  # qualitativedata <<- qualitativedata
  qualitativedata <- getmetainfo(data = qualitativedata,idlist = "cid",cid = T,needlist = c("HMDB","KEGG"))
  qualitativedata[!is.na(qualitativedata$KEGG),"level"] <- "*"
  ...
  if (file.exists(lcqualitativefrom)) {
    ...
    qualitativedata[qualitativedata$HMDB %in% mandata$id,"level"] <- "**"
    qualitativedata[qualitativedata$cid %in% mandata$id,"level"] <- "**"
  }else{
  ...
  if (file.exists(manqualitativefrom)) {
    ...
    qualitativedata[qualitativedata$HMDB %in% mandata$`Compound ID`,"level"] <- "***"
    qualitativedata[qualitativedata$cid %in% mandata$`Compound ID`,"level"] <- "***"
  }else{
  ...
  for ( i in 1:length(formuladata)) {
    ...
    if(dim(qualitativedata2)[1] > 1){
      ...
      if(dim(qualitativedata2)[1] > 3){
        if(any(qualitativedata2$`HMDB` %in% species_tissuelocation)){
          qualitativedata2 <- qualitativedata2[qualitativedata2$`HMDB` %in% species_tissuelocation,]
        }
      }
      
      if(dim(qualitativedata2)[1] > 3){
        if(any(!is.na(qualitativedata2$HMDB))){
          qualitativedata2 <- qualitativedata2[!is.na(qualitativedata2$HMDB),]
        }
      }
      
      if(dim(qualitativedata2)[1] > 3){
        qualitativedata2 <- qualitativedata2[order(qualitativedata2$`cid`),]
  ...
  qualitativedata3 <- qualitativedata3[order(qualitativedata3$mz),]
...
if(this.path::is.main() & !is.null(whereami::thisfile_r())){
  ...
  result <- do.call(what = imzmlpreQualitative_2023,args = args)
