...
configfile: "config_loadingmap2.yaml"
...
# 验证config
validate(config, "config_loadingmap2.schema.yaml")

# if not os.path.exists(config["path"]["config_savepath"]): 
# 	os.makedirs(config["path"]["config_savepath"])
# with open("{path}/config_mulstatistic_plot_loadingmap2.yaml".format(path=config["path"]["config_savepath"]),"w",encoding="utf-8") as f:
# 	yaml.dump(config,f,allow_unicode=True,sort_keys=False)

rule loadingmap2_all:
    input:
...
rule loadingmap2:
    ...
    output:
        "{savepath}/{mode,(OPLS-DA)|(PLS-DA)|(PCA)}-loading2-{compare}.{imagetype,(pdf)|(jpg)|(png)|(tiff)|(html)}"
    params:
    ...
    shell:
        '''
        map_mulstatistics_loadingmap2 \
        --filename '{input}' \
        --savepath '{wildcards.savepath}' \
        --mapname '{wildcards.mode}-loading2-{wildcards.compare}' \
