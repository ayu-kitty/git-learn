...
configfile: "config_mulstatistic_pca.yaml"
...
# 验证config
validate(config, "config_mulstatistic_pca.schema.yaml")

# if not os.path.exists(config["path"]["config_savepath"]): 
# 	os.makedirs(config["path"]["config_savepath"])
# with open("{path}/config_mulstatistic_ana_pca.yaml".format(path=config["path"]["config_savepath"]),"w",encoding="utf-8") as f:
# 	yaml.dump(config,f,allow_unicode=True,sort_keys=False)

rule pca_all:
    input:
        expand("{mulstatistics_rds_savepath}/mulstatistic_PCA-{compare}.rds",
...
if config["params"]["mulboth"]:
  ...
  rule pca:
      ...
      output:
          rdssavepath=config["path"]["mulstatistics_rds_savepath"]+"/mulstatistic_PCA-{compare2,([A-Za-z0-9._+-]*-vs-[A-Za-z0-9._+-]*)|(All)|(Allsample)}.rds",
          rdssavepathlc=config["path"]["mulstatistics_rds_savepath"]+"/mulstatistic_PCA-{compare2,([A-Za-z0.-9_+-]*-vs-[A-Za-z0-9._+-]*)|(All)|(Allsample)}~LCMS.rds",
          rdssavepathgc=config["path"]["mulstatistics_rds_savepath"]+"/mulstatistic_PCA-{compare2,([A-Za-z0-9._+-]*-vs-[A-Za-z0-9._+-]*)|(All)|(Allsample)}~GCMS.rds"
      params:
  ...
  rule pca:
      input:
          ...
          classfile=lambda wildcards: config["path"]["classfile"]
      output:
          rdssavepath=config["path"]["mulstatistics_rds_savepath"]+"/mulstatistic_PCA-{compare,([A-Za-z0-9._+-]*-vs-[A-Za-z0-9._+-]*)|(All)|(Allsample)}.rds"
      params:
