...
configfile: "config_mulstatistic_pls.yaml"
...
# 验证config
validate(config, "config_mulstatistic_pls.schema.yaml")

# if not os.path.exists(config["path"]["config_savepath"]): 
# 	os.makedirs(config["path"]["config_savepath"])
# with open("{path}/config_mulstatistic_ana_pls.yaml".format(path=config["path"]["config_savepath"]),"w",encoding="utf-8") as f:
# 	yaml.dump(config,f,allow_unicode=True,sort_keys=False)

rule pls_all:
    input:
        expand("{mulstatistics_rds_savepath}/mulstatistic_PLS-DA-{compare}.rds",
...
if config["params"]["mulboth"]:
  rule pls:
      ...
      output:
          rdssavepath=config["path"]["mulstatistics_rds_savepath"]+"/mulstatistic_PLS-DA-{compare2,([A-Za-z0-9._+-]*-vs-[A-Za-z0-9._+-]*)|(All)|(Allsample)}.rds",
          rdssavepathlc=config["path"]["mulstatistics_rds_savepath"]+"/mulstatistic_PLS-DA-{compare2,([A-Za-z0-9._+-]*-vs-[A-Za-z0-9._+-]*)|(All)|(Allsample)}~LCMS.rds",
          rdssavepathgc=config["path"]["mulstatistics_rds_savepath"]+"/mulstatistic_PLS-DA-{compare2,([A-Za-z0-9._+-]*-vs-[A-Za-z0-9._+-]*)|(All)|(Allsample)}~GCMS.rds"
      params:
  ...
  rule pls:
      input:
          ...
          classfile=lambda wildcards: config["path"]["classfile"]
      output:
          rdssavepath=config["path"]["mulstatistics_rds_savepath"]+"/mulstatistic_PLS-DA-{compare,([A-Za-z0-9._+-]*-vs-[A-Za-z0-9._+-]*)|(All)|(Allsample)}.rds"
      params:
